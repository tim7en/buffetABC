...
MODELS_PKL = DATA_DIR / "models.pkl"    # persisted final-month models
MODEL_RETRAIN_DAYS = 28                  # retrain if pkl is older than this
...
def _models_pkl_is_fresh() -> bool:
    """Return True if models.pkl exists and is <= MODEL_RETRAIN_DAYS old."""
    if not MODELS_PKL.exists():
        return False
    age_days = (datetime.now(timezone.utc) - datetime.fromtimestamp(
        MODELS_PKL.stat().st_mtime, tz=timezone.utc
    )).days
    return age_days <= MODEL_RETRAIN_DAYS


def _save_models_pkl(spy_models: dict, qqq_models: dict) -> None:
    """Persist the final-month fitted models to disk."""
    MODELS_PKL.parent.mkdir(parents=True, exist_ok=True)
    payload = {
        "spy": spy_models,
        "qqq": qqq_models,
    ...
    joblib.dump(payload, MODELS_PKL, compress=3)
    log.info("Saved models.pkl  (spy_through=%s  qqq_through=%s)",
             spy_models["trained_through"], qqq_models["trained_through"])
...
def _load_models_pkl() -> dict:
    ...
    for asset_key in ("spy", "qqq"):
        if asset_key not in payload:
            continue
        for model_key in ("risk_off_logistic", "risk_off_rf", "jump_in_logistic", "jump_in_rf"):
            if model_key in payload[asset_key]:
                _disable_parallelism(payload[asset_key][model_key])
    return payload


def compute_signals() -> dict[str, Any]:
    """Run the SPY-gate × QQQ-ensemble pipeline and return a state dict.

    This function is called:
      • Once at container startup (if the cached state is stale / missing)
      • Every trading day at ~17:30 ET by the scheduler

    Fast path  (models.pkl <= MODEL_RETRAIN_DAYS old):
      Download fresh data → compute features → predict with saved models.
      Skips the full walk-forward training loop (~15 min → ~2 min).

    Full-retrain path  (pkl missing or stale):
      Walk-forward training → predict → save new models.pkl.

    Returns a dict matching the ``SignalState`` dataclass fields.
    """
    computed_at = datetime.now(timezone.utc).isoformat()
    use_fast_path = _models_pkl_is_fresh()
    log.info("=== Signal computation started at %s  (path=%s) ===",
             computed_at, "fast-inference" if use_fast_path else "full-retrain")

    try:
        if use_fast_path:
            # ---------------------------------------------------------------- #
            # FAST PATH — load saved models, compute only today's features     #
            # ~2 min instead of ~15 min                                        #
            # ---------------------------------------------------------------- #
            saved = _load_models_pkl()
            log.info("Loaded models.pkl  (saved_at=%s)", saved.get("saved_at_utc", "?"))

            # Build fresh datasets (prices + macro already on disk, fast)
            spy_data = ra.load_qqq(SPY_PARQUET, start=PRICE_START, end=None)
            qqq_data = ra.load_qqq(QQQ_PARQUET, start=PRICE_START, end=None)
            spy_macro = ra.load_macro(
                MACRO_PARQUET, spy_data.index,
                monthly_release_lag_days=MONTHLY_LAG_DAYS,
                quarterly_release_lag_days=QUARTERLY_LAG_DAYS,
                annual_release_lag_days=ANNUAL_LAG_DAYS,
            )
            qqq_macro = ra.load_macro(
                MACRO_PARQUET, qqq_data.index,
                monthly_release_lag_days=MONTHLY_LAG_DAYS,
                quarterly_release_lag_days=QUARTERLY_LAG_DAYS,
                annual_release_lag_days=ANNUAL_LAG_DAYS,
            )
            spy_stress, _ = ra.load_stress_proxies(spy_data.index, FRED_CACHE_DIR, refresh=True)
            qqq_stress, _ = ra.load_stress_proxies(qqq_data.index, FRED_CACHE_DIR, refresh=False)
            ...
            # Get most-recent month-end feature row and predict
            def _latest_features(monthly: pd.DataFrame, models: dict) -> tuple[pd.DataFrame, pd.Timestamp]:
                use_features = models["features"]
                for ts in reversed(monthly.index.tolist()):
                    row = monthly.loc[[ts], use_features].replace([np.inf, -np.inf], np.nan)
                    if not row.isna().any(axis=None):
                        return row, ts
                raise RuntimeError("No valid feature row found in monthly sample.")

            spy_feat, spy_ts = _latest_features(spy_monthly, saved["spy"])
            qqq_feat, qqq_ts = _latest_features(qqq_monthly, saved["qqq"])

            spy_pred = mc.predict_from_models(saved["spy"], spy_feat,
                                              RISK_OFF_THRESHOLD, JUMP_IN_THRESHOLD)
            qqq_pred = mc.predict_from_models(saved["qqq"], qqq_feat,
                                              RISK_OFF_THRESHOLD, JUMP_IN_THRESHOLD)

            # Build full daily signals via ffill + 1-day lag (matches backtest)
            spy_daily_s = pd.Series({spy_ts: spy_pred["regime"]})
            qqq_daily_s = pd.Series({qqq_ts: qqq_pred["regime"]})
            spy_daily = mc.monthly_signal_to_daily(spy_daily_s, spy_dataset.index)
            qqq_daily = mc.monthly_signal_to_daily(qqq_daily_s, qqq_dataset.index)

            spy_ro_prob   = spy_pred["risk_off_probability"]
            spy_ji_prob   = spy_pred["jump_in_probability"]
            qqq_ro_prob   = qqq_pred["risk_off_probability"]
            qqq_ji_prob   = qqq_pred["jump_in_probability"]
            spy_trained_through = saved["spy_trained_through"]
            qqq_trained_through = saved["qqq_trained_through"]
            log.info("Fast-path predictions: SPY=%s (ro=%.2f)  QQQ=%s (ro=%.2f)",
                     spy_pred["regime"], spy_ro_prob,
                     qqq_pred["regime"], qqq_ro_prob)

        else:
            # ---------------------------------------------------------------- #
            # FULL RETRAIN PATH — walk-forward training + save pkl             #
            # ---------------------------------------------------------------- #
            ...
            qqq_daily, qqq_blend, qqq_dataset = _build_signals_for(
                QQQ_PARQUET, "QQQ", refresh_fred=False
            )
            ...
            # Also fit and save final-month models for future fast-path runs
            try:
                spy_monthly = ra.month_end_sample(spy_dataset)
                qqq_monthly = ra.month_end_sample(qqq_dataset)
                spy_final = mc.fit_final_models(
                ...
                _save_models_pkl(spy_final, qqq_final)
            except Exception as pkl_exc:
                log.warning("Could not save models.pkl (non-fatal): %s", pkl_exc)

        # -------------------------------------------------------------------- #
        # Gate + policy — common for both paths                                 #
        # -------------------------------------------------------------------- #
        ...
        history = [
            {
                ...
                "policy_signal": str(s),
                "spy_signal": (str(spy_daily.reindex([d]).iloc[0])
                               if d in spy_daily.index else "unknown"),
                "qqq_signal": (str(qqq_daily.reindex([d]).iloc[0])
        ...
        log.info(
            "=== Computation complete: %s (SPY=%s, QQQ=%s, VIX=%.1f) ===",
