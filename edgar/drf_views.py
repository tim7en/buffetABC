...
class StrategyViewSet(viewsets.ViewSet):
    ...
    def backtest_intraday(self, request):
        ...
        try:
            if strategy_variant == "hourly_sfp_fvg":
            ...
            elif strategy_variant == "session_range_breakout":
                payload = run_session_range_breakout_backtest(
                    ...
                    market_data_symbol=(request.data.get("market_data_symbol") or "").strip().upper() or None,
                    auto_adjust_for_yf_limits=_as_bool(request.data.get("auto_adjust_for_yf_limits"), True),
                    session_open=(request.data.get("session_open") or "new_york_equity_open").strip().lower(),
                    allow_longs=allow_longs,
                    allow_shorts=allow_shorts,
                    range_lookback_minutes=int(request.data.get("range_lookback_minutes", 180)),
                    breakout_window_minutes=int(request.data.get("breakout_window_minutes", 90)),
                    breakout_buffer_bps=float(request.data.get("breakout_buffer_bps", 0.0)),
                    breakout_close_buffer_bps=float(request.data.get("breakout_close_buffer_bps", 0.0)),
                    use_target_room_filter=_as_bool(request.data.get("use_target_room_filter"), False),
                    min_target_room_ratio=float(request.data.get("min_target_room_ratio", 1.0)),
                    stop_buffer_bps=float(request.data.get("stop_buffer_bps", 5.0)),
                    rr_multiple=float(request.data.get("rr_multiple", 2.0)),
                    volume_period=int(request.data.get("volume_period", 40)),
                ...
                payload = run_orb_turtle_hybrid_backtest(
                    ...
                    allow_longs=allow_longs,
                    allow_shorts=allow_shorts,
                    orb_window_minutes=int(request.data.get("orb_window_minutes", 15)),
                    use_five_minute_or=_as_bool(request.data.get("use_five_minute_or"), False),
                    orb_entry_window_minutes=int(request.data.get("orb_entry_window_minutes", 120)),
                    orb_break_buffer_bps=float(request.data.get("orb_break_buffer_bps", 0.0)),
                    donchian_period=int(request.data.get("donchian_period", 200)),
                    ema_period=int(request.data.get("ema_period", 200)),
                    daily_ema_period=int(request.data.get("daily_ema_period", 5)),
                    rvol_period=int(request.data.get("rvol_period", 40)),
                    min_rel_volume=float(request.data.get("min_rel_volume", 1.0)),
                    long_risk_pct=float(request.data.get("long_risk_pct", 0.015)),
                    short_risk_pct=float(request.data.get("short_risk_pct", 0.03)),
                    max_position_pct=float(request.data.get("max_position_pct", 0.30)),
                    stop_buffer_bps=float(request.data.get("stop_buffer_bps", 5.0)),
                    orb_rr_multiple=float(request.data.get("orb_rr_multiple", 2.0)),
                    short_time_stop_minutes=int(request.data.get("short_time_stop_minutes", 120)),
                    ...
                    short_meta_lookback=int(request.data.get("short_meta_lookback", 4)),
                    short_meta_loss_threshold=int(request.data.get("short_meta_loss_threshold", 2)),
                    short_meta_risk_scale=float(request.data.get("short_meta_risk_scale", 0.5)),
                    slippage_bps=float(request.data.get("slippage_bps", 4.0)),
                    commission_bps=float(request.data.get("commission_bps", 1.0)),
                    exit_fill_policy=exit_fill_policy,
                )
