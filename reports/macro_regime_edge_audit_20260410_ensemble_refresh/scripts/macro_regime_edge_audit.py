...
def prediction_from_monthly_export(monthly_regimes: pd.DataFrame, model_name: str) -> pd.Series:
    model_regimes = monthly_regimes[monthly_regimes["model_name"] == model_name].copy()
    if model_regimes.empty:
        return pd.Series(dtype=object, name=model_name)
    return model_regimes["regime"].dropna().rename(model_name)
...
def main() -> None:
    ...
    logistic_prediction = prediction_from_monthly_export(monthly_regimes, "logistic")
    gmm_monthly = regime_month_end_from_daily(gmm_daily, "wf_gmm_regime", "gmm")
    gmm_prediction = gmm_monthly["regime"].dropna().rename("gmm")
    ...
    for model_name in ["random_forest", "ensemble_blend", "ensemble_majority"]:
        prediction = prediction_from_monthly_export(monthly_regimes, model_name)
        if not prediction.empty:
            prediction_map[model_name] = prediction
    ...
    for model_name in ["random_forest", "ensemble_blend", "ensemble_majority"]:
        column = f"{model_name}_signal_lag1"
        if column in daily_signals:
            leverage_signal_map[model_name] = daily_signals[column]
    ...
    metadata = {
        ...
        "notes": [
            ...
            "Random forest and ensemble rows are included when the compare package exports them; ensemble variants only combine existing walk-forward signals.",
