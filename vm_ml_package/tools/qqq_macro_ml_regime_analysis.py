...
# matplotlib is imported lazily inside each plot_* function to avoid
# font-manager threading issues in headless/containerised environments.
import numpy as np
import pandas as pd
...
MODEL_FEATURES = [
    ...
    "nfci_63d_change",
    "t10y3m_level",
    "sector_rotation_xlu_xly",
    "sector_rotation_63d_change",
    "copper_gold_ratio_level",
    "copper_gold_63d_return",
    "eem_63d_return",
    "efa_63d_return",
    "vix_term_structure",
    "vix_term_structure_21d_change",
]
...
FEATURE_LABELS = {
    ...
    "sector_rotation_xlu_xly": "Sector rotation XLU/XLY ratio",
    "sector_rotation_63d_change": "Sector rotation 3-month change",
    "copper_gold_ratio_level": "Copper / Gold ratio",
    "copper_gold_63d_return": "Copper / Gold 3-month return",
    "eem_63d_return": "EM equity 3-month return",
    "efa_63d_return": "DM ex-US equity 3-month return",
    "vix_term_structure": "VIX / VIX3M term structure",
    "vix_term_structure_21d_change": "VIX term structure 1-month change",
}
...
def build_dataset(
    ...
    # --- v2 features: breadth, cross-asset, vol-structure ---
    # Sector rotation: XLU (defensives) vs XLY (cyclicals) — rising = risk-off rotation
    xlu = macro.get("xlu")
    xly = macro.get("xly")
    if xlu is not None and xly is not None:
        xlu_xly_ratio = xlu / xly.replace(0.0, np.nan)
        df["sector_rotation_xlu_xly"] = xlu_xly_ratio
        df["sector_rotation_63d_change"] = xlu_xly_ratio.diff(63)
    else:
        df["sector_rotation_xlu_xly"] = np.nan
        df["sector_rotation_63d_change"] = np.nan

    # Copper / Gold ratio — rising = growth optimism, falling = risk-off
    copper = macro.get("copper")
    gold = macro.get("gold")
    if copper is not None and gold is not None:
        cu_au_ratio = copper / gold.replace(0.0, np.nan)
        df["copper_gold_ratio_level"] = cu_au_ratio
        df["copper_gold_63d_return"] = _pct_change(cu_au_ratio, 63)
    else:
        df["copper_gold_ratio_level"] = np.nan
        df["copper_gold_63d_return"] = np.nan

    # International equity momentum — global risk-off leads US
    eem = macro.get("eem")
    efa = macro.get("efa")
    df["eem_63d_return"] = _pct_change(eem, 63) if eem is not None else np.nan
    df["efa_63d_return"] = _pct_change(efa, 63) if efa is not None else np.nan

    # VIX term structure — VIX/VIX3M > 1.0 = backwardation (panic), < 1.0 = contango (normal)
    vix3m = macro.get("vix3m")
    # Note: stress["vix"] is used below; use it here too for consistency
    if vix3m is not None:
        vix_daily = stress["vix"] if "vix" in stress.columns else macro.get("vix")
        if vix_daily is not None:
            vix_ts = vix_daily / vix3m.replace(0.0, np.nan)
            df["vix_term_structure"] = vix_ts
            df["vix_term_structure_21d_change"] = vix_ts.diff(21)
        else:
            df["vix_term_structure"] = np.nan
            df["vix_term_structure_21d_change"] = np.nan
    else:
        df["vix_term_structure"] = np.nan
        df["vix_term_structure_21d_change"] = np.nan
    ...
    # v2: VIX backwardation spike and sector rotation into defensives signal stress
    if df["vix_term_structure"].notna().sum() >= 252:
        shock_parts.append(_rolling_zscore(df["vix_term_structure"]).clip(lower=0.0))
    if df["sector_rotation_63d_change"].notna().sum() >= 252:
        shock_parts.append(_rolling_zscore(df["sector_rotation_63d_change"]).clip(lower=0.0))
    ...
    # v2: copper/gold rising = growth optimism, EM momentum = global risk-on
    if df["copper_gold_63d_return"].notna().sum() >= 252:
        sentiment_parts.append(_rolling_zscore(df["copper_gold_63d_return"]))
    if df["eem_63d_return"].notna().sum() >= 252:
        sentiment_parts.append(_rolling_zscore(df["eem_63d_return"]))
...
def variable_source(variable: str) -> str:
    ...
    if variable.startswith("t10y3m_"):
        return "FRED T10Y3M"
    if variable.startswith("sector_rotation_"):
        return "Derived from Yahoo XLU / XLY ETFs"
    if variable.startswith("copper_gold_"):
        return "Derived from Yahoo HG=F copper / GC=F gold"
    if variable.startswith("eem_"):
        return "Yahoo EEM (iShares MSCI EM)"
    if variable.startswith("efa_"):
        return "Yahoo EFA (iShares MSCI EAFE)"
    if variable.startswith("vix_term_structure"):
        return "Derived from FRED VIXCLS / Yahoo ^VIX3M"
...
def variable_frequency(variable: str) -> str:
    if variable.startswith(("dxy_", "gold_", "wilshire_", "us2y_", "us10y_", "us30y_", "curve_", "wti_", "vix_", "hy_oas_", "nfci_", "t10y3m_", "qqq_", "sector_rotation_", "copper_gold_", "eem_", "efa_")):
        return "daily"
