...
def fit_final_models(
    ...
    *,
    ...
    """Fit only the most-recent valid month-end models without the full walk-forward loop.

    Walks backwards from the end of *sample* to find the latest month-end that
    has (a) non-NaN features and (b) at least *min_train_months* complete prior
    observations.  Fits four sklearn pipelines:
        risk_off  logistic, jump_in  logistic
        risk_off  random_forest, jump_in  random_forest
    Returns a dict that can be passed directly to ``joblib.dump()``::

        {
            "risk_off_logistic": fitted_pipeline,
            "jump_in_logistic":  fitted_pipeline,
            "risk_off_rf":       fitted_pipeline,
            "jump_in_rf":        fitted_pipeline,
            "features":          [str, ...],
            "trained_through":   "YYYY-MM-DD",
            "train_n":           int,
            "risk_off_prob":     float,   # blended probability
            "jump_in_prob":      float,
    ...
    if len(use_features) < 6:
        raise RuntimeError(
            f"Only {len(use_features)} usable features after filtering; need at least 6."
        )
    ...
    for i in range(len(sample) - 1, -1, -1):
        ...
        # Fit all four pipelines on the full available training set
        ro_logistic = classifier_pipeline("logistic", random_state, rf_estimators)
        ro_logistic.fit(risk_off_train[use_features], risk_off_train["risk_off_target"].astype(int))

        ji_logistic = classifier_pipeline("logistic", random_state, rf_estimators)
        ji_logistic.fit(jump_in_train[use_features], jump_in_train["jump_in_target"].astype(int))

        ro_rf = classifier_pipeline("random_forest", random_state, rf_estimators)
        ro_rf.fit(risk_off_train[use_features], risk_off_train["risk_off_target"].astype(int))

        ji_rf = classifier_pipeline("random_forest", random_state, rf_estimators)
        ji_rf.fit(jump_in_train[use_features], jump_in_train["jump_in_target"].astype(int))

        # Predict for this date (ensemble blend)
        ro_prob = (
            float(ro_logistic.predict_proba(current[use_features])[:, 1][0])
            + float(ro_rf.predict_proba(current[use_features])[:, 1][0])
        ) / 2.0
        ji_prob = (
            float(ji_logistic.predict_proba(current[use_features])[:, 1][0])
            + float(ji_rf.predict_proba(current[use_features])[:, 1][0])
        ) / 2.0

        return {
            "risk_off_logistic": ro_logistic,
            "jump_in_logistic":  ji_logistic,
            "risk_off_rf":       ro_rf,
            "jump_in_rf":        ji_rf,
            "features":          list(use_features),
            "trained_through":   date.date().isoformat(),
            "train_n":           len(risk_off_train),
            "risk_off_prob":     ro_prob,
            "jump_in_prob":      ji_prob,
            "regime":            regime_from_probabilities(
                ro_prob, ji_prob, risk_off_threshold, jump_in_threshold
            ),
        }

    raise RuntimeError("Could not find a valid training window in the sample.")
...
def predict_from_models(
    ...
    """Run predict_proba on four pre-fitted pipelines for a single observation.

    *current_features* must be a single-row DataFrame with at least the columns
    listed in ``models["features"]``.
    ...
    row = current_features[use_features].replace([np.inf, -np.inf], np.nan)

    ro_prob = (
        float(models["risk_off_logistic"].predict_proba(row)[:, 1][0])
        + float(models["risk_off_rf"].predict_proba(row)[:, 1][0])
    ) / 2.0
    ji_prob = (
        float(models["jump_in_logistic"].predict_proba(row)[:, 1][0])
        + float(models["jump_in_rf"].predict_proba(row)[:, 1][0])
    ) / 2.0

    return {
        "risk_off_probability": ro_prob,
        "jump_in_probability":  ji_prob,
        "regime": regime_from_probabilities(ro_prob, ji_prob, risk_off_threshold, jump_in_threshold),
    }
