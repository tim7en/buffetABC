"""
Run the big-tech theme allocator with an added energy sleeve.

Universe change:
  remove: LTC-USD, ENJ-USD, BCH-USD, ATOM-USD
  add   : GOOGL, META, NVDA, BRENT, NATGAS

Variants:
1. EMA cutoff base with explicit theme caps
2. EMA cutoff base + bucket/theme allocator

Energy is treated as a separate sleeve with its own bucket cap and allocator
multiplier path.
"""
...
ADDED_SYMBOLS = ADDED_EQUITIES + ADDED_ENERGY

OUTPUT_DIR = Path("reports/strategy_health_audit/bigtech_theme_allocator_energy")
...
THEME_BUCKETS = ("crypto", "gold", "metals", "energy", "equity")
...
def _extend_universe() -> tuple[tuple[str, str, str], ...]:
    extra = [(ticker, "tiingo", "new_york_equity_open") for ticker in ADDED_SYMBOLS]
    ...
    stp.ENERGY_TICKERS = set(stp.ENERGY_TICKERS) | set(ADDED_ENERGY)
...
def _run_variant(
    *,
    ...
    summary["added_energy"] = list(ADDED_ENERGY)
    if extra_summary:
...
def main() -> None:
    ...
    themed = _run_variant(
        label="Theme allocator + energy sleeve + GOOGL/META/NVDA",
    ...
    comparison = {
        row["variant_label"]: {
            ...
            "added_energy": list(ADDED_ENERGY),
    ...
    print("\n" + "=" * 104)
    print("BIG TECH + ENERGY THEME ALLOCATOR COMPARISON")
    print("=" * 104)
    print(f"{'Variant':<50} {'Return %':>10} {'CAGR %':>9} {'MaxDD %':>9} {'PF':>6} {'WR %':>7} {'Trades':>8}")
    print("-" * 104)
    for row in rows:
        print(
            f"{row['variant_label']:<50} {row['total_return_pct']:>10.2f} {row['cagr_pct']:>9.2f} "
